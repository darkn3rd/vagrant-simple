...
_config = os.environ.get('VAGRANT_CONFIG') or '{}/global.ini'.format(_script_home)
...
def get_ssh_config():
    ...
    parser.read(_config)
    # fetch hosts section from INI file
    hosts = parser.items('hosts') # [('hostname', 'ip'),('hostname','ip')]

    # build hostvars dictionary from global ini file
    for hostname, ip in hosts:
          ansible_config = {}
          ansible_config['ansible_ssh_private_key_file'] = _sshkey.format(hostname)
          ansible_config['ansible_ssh_user'] = 'vagrant'
          ansible_config['ansible_ssh_host'] = ip[1:-1] # strip literal quote chars
...
def get_a_ssh_config(box_name):
    ...
    # fetch box_name from hosts section in INI file
    ip = parser.get('hosts', box_name)

    # build single ansible connection
