...
def get_ssh_config():
    ...
    # build hostvars dictionary from csv file
    with open(_config) as csvfile:
...
def get_a_ssh_config(box_name):
    ...
    # find row with matching hostname
    with open(_config) as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if row['hostname'] == box_name:
                break

    # build return structure given ipaddr column of last row fetch
